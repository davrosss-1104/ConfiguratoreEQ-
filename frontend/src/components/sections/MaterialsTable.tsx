...
export const MaterialsTable = ({ 
  ...
  // Ref per evitare toast duplicati
  ...
  // Toast per materiali con codice articolo non trovato
  useEffect(() => {
    const nonTrovati = materials.filter((m: any) => m.articolo_non_trovato);
    if (nonTrovati.length > 0 && nonTrovati.length !== lastNotifiedCountRef.current) {
      lastNotifiedCountRef.current = nonTrovati.length;
      toast.warning(
        `${nonTrovati.length} materiale${nonTrovati.length > 1 ? 'i' : ''} con codice articolo non trovato`,
        {
          description: nonTrovati.map((m: any) => m.codice).join(', '),
          duration: 6000,
        }
      );
      ...
      <div className="border-b px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {(() => {
              const n = materials.filter((m: any) => m.articolo_non_trovato).length;
              return n > 0 ? (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {(material as any).articolo_non_trovato && (
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-red-100 text-red-700" title="Codice articolo non presente in anagrafica">
                            ⚠ Cod. non trovato
                          </span>
                        )}
                      </div>
