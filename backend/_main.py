...
def evaluate_rules(preventivo_id: int, db: Session) -> dict:
    ...
    for rule in rules:
        ...
        all_conditions_met = all(evaluate_condition(c, config_data) for c in conditions)
            ...
            for mat in rule.get("materials", []):
                ...
                if not existing:
                    nuovo = Materiale(
                        ...
                        prezzo_unitario=mat.get("prezzo_unitario", 0),
                        prezzo_totale=mat.get("quantita", 1) * mat.get("prezzo_unitario", 0),
...
def update_normative(preventivo_id: int, data: NormativeUpdate, db: Session = Depends(get_db)):
    ...
    en_81_20_precedente = dati.en_81_20 if dati else None
    ...
    # Se EN 81-20 è cambiato, gestisci materiali
    nuovo_en_81_20 = dati.en_81_20
    
    # Se era 2020 e ora non lo è più, rimuovi materiali
    if en_81_20_precedente == '2020' and nuovo_en_81_20 != '2020':
        deleted = db.query(Materiale).filter(
            Materiale.preventivo_id == preventivo_id,
            Materiale.regola_id == "RULE_EN81_20_2020"
        ).delete()
        materials_removed = deleted
        db.commit()
    
    # Se è 2020, aggiungi materiali (se non esistono già)
    
    
    # Ricalcola totale se ci sono state modifiche
    ...
    # Ritorna dati con info materiali
    result = {
        "id": dati.id,
        "preventivo_id": dati.preventivo_id,
        "en_81_1": dati.en_81_1,
        "en_81_20": dati.en_81_20,
        "en_81_21": dati.en_81_21,
        "en_81_28": dati.en_81_28,
        "en_81_70": dati.en_81_70,
        "en_81_72": dati.en_81_72,
        "en_81_73": dati.en_81_73,
        "a3_95_16": dati.a3_95_16,
        "dm236_legge13": dati.dm236_legge13,
        "emendamento_a3": dati.emendamento_a3,
        "uni_10411_1": dati.uni_10411_1,
        "materials_added": materials_added,
        "materials_removed": materials_removed,
    }
...
def convert_rule_designer_to_configurator(rd_rule: dict, db: Session = None) -> dict:
    ...
    materials = []
    output = rd_rule.get("output", {})
    quantita_config = rd_rule.get("quantita", {})
        ...
        if db:
            try:
                ...
                if articolo:
                    ...
                    prezzo = float(articolo.prezzo_listino or 0)
            except Exception:
                pass
