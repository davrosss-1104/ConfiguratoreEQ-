...
def extract_number(value) -> float:
    """Estrae il numero da una stringa come '48Vcc', '80V', '3.5kW'"""
    if value is None:
        return 0.0
    if isinstance(value, (int, float)):
        return float(value)
    # Estrai solo i caratteri numerici e il punto decimale
...
def evaluate_condition(condition: dict, config_data: dict) -> bool:
    ...
    if field not in config_data:
        print(f"        ❌ Campo '{field}' non presente in config_data")
        return False
    
    actual_value = config_data.get(field)
    print(f"        🔎 {field}: '{actual_value}' {operator} '{expected_value}'")
    
    if actual_value is None or actual_value == "":
        print(f"        ❌ Valore nullo o vuoto")
        return False
    
    result = False
    if operator == "equals":
        result = str(actual_value).lower() == str(expected_value).lower()  # Case insensitive
        ...
        # Estrai numeri da stringhe come "48Vcc", "80V", "3.5kW"
        actual_num = extract_number(actual_value)
        expected_num = extract_number(expected_value)
        result = actual_num > expected_num
        print(f"        📊 Confronto numerico: {actual_num} > {expected_num}")
        ...
        result = str(actual_value) in expected_value
    
    print(f"        {'✅' if result else '❌'} Risultato: {result}")
    return result
...
def evaluate_rules(preventivo_id: int, db: Session) -> dict:
    ...
    """
    print(f"\n{'='*50}")
    print(f"🔍 EVALUATE_RULES per preventivo {preventivo_id}")
    print(f"{'='*50}")
    ...
    config_data = {}
    if dati_principali:
        # Raccogli TUTTI i campi da dati_principali
        for col in dati_principali.__table__.columns:
            if col.name not in ['id', 'preventivo_id', 'created_at', 'updated_at']:
                config_data[col.name] = getattr(dati_principali, col.name, None)
    if normative:
        # Raccogli TUTTI i campi da normative
        for col in normative.__table__.columns:
            if col.name not in ['id', 'preventivo_id', 'created_at', 'updated_at']:
                config_data[col.name] = getattr(normative, col.name, None)
    if argano:
        # Raccogli TUTTI i campi da argano
        for col in argano.__table__.columns:
            if col.name not in ['id', 'preventivo_id', 'created_at', 'updated_at']:
                config_data[col.name] = getattr(argano, col.name, None)
    
    print(f"📋 CONFIG DATA: {config_data}")
    
    # Carica e valuta regole
    rules = load_rules()
    print(f"📁 Regole caricate: {len(rules)}")
    ...
    for rule in rules:
        ...
        if not conditions and not rule.get("is_default", False):
            print(f"    ⚠️ Saltata (nessuna condizione)")
            continue
        ...
        print(f"    ✅ Condizioni soddisfatte: {all_conditions_met}")
...
@app.put("/api/preventivi/{preventivo_id}/dati-principali")
def update_dati_principali(preventivo_id: int, data: DatiPrincipaliUpdate, db: Session = Depends(get_db)):
    ...
    rules_result = evaluate_rules(preventivo_id, db)
    
    # Restituisci dati + risultato regole
    result = {
        **{c.name: getattr(dati, c.name) for c in dati.__table__.columns},
        "materials_added": rules_result.get("materials_added", 0),
        "materials_removed": rules_result.get("materials_removed", 0),
...
@app.put("/api/preventivi/{preventivo_id}/normative")
